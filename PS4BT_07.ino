...
class PID {
  ...
  public:
    ...
    double process() {
      // ImplementaÃƒÂ§ÃƒÂ£o P ID
      error = setPoint - sample;
      float deltaTime = (millis() - lastProcess) / 100;
      lastProcess = millis();
...
//double temperature = map(ax, -660, 660, 600, 2000);
  double roll = ax;
  double pitch = ay;
  double yaw = az;
  Pidroll.addNewSample(roll);
  Pidpitch.addNewSample(pitch);
  Pidyaw.addNewSample(yaw);

  // convierte para el control
  controlePwm = map(Pidroll.process(), 0, 660, 600, 2000);
  controlePwm = map(Pidpitch.process(), 0, 660, 600, 2000);
  controlePwm = map(Pidyaw.process(), 0, 660, 600, 2000);
